...
public class MainActivity extends ActionBarActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final String str = etSearch.getText().toString();
        ...
        btnSearch.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                //进行匹配排序
                String[] sort = sort(destination);
                lvResult.setAdapter(new ArrayAdapter<String>(MainActivity.this, android.R.layout.simple_list_item_1, sort));
            }
        });

    }

    public String[] sort(String[] split) {
            ...
            if (destination != null) {
                for (int i = 0; i < array2D[arr2D].length; i++) {
                    for (int j = 0; j < destination.length; j++) {
                        if (array2D[arr2D][i].equals(destination[j])) {
                            count[arr2D]++;
                        }
                    }
                }
